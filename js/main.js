...
const setPosts = {
	...
	*/
	addPost(title, text, tags, handler) {
		...
		// Установить ключ к БД и отправить в неё все посты.
	...
	// Метод получает посты из БД Firebase. Принимает callback-функцию.
	getPosts(handler) {
		// Получить копию постов из БД.
		firebase.database().ref('post').on('value', snapshot => {
			// Записать полученные посты в хранилище всех постов.
